...
PosterQuangCao::PosterQuangCao() {
    ...
    parts.push_back(new TransformShape(Translate(0.4f, -0.15f, zF + 0.01f) * Scale(0.5f, 1.0f, 0.05f), new Cube()));
    ...
    parts.push_back(new TransformShape(Translate(-0.4f, 0.1f, zF) * Scale(0.2f, 0.4f, 0.05f), new Cube()));
    ...
    parts.push_back(new TransformShape(Translate(0.4f, -0.7f, zF + 0.02) * Scale(0.25f, 0.25f, 0.07f), new Cube()));
}
...
void PosterQuangCao::draw(const mat4& modelMatrix) const {
    for (size_t i = 0; i < parts.size(); ++i) {
        if (i == 0) Materials::ConcreteBeige.apply();      // N?n Poster tr?ng
        else if (i == 1) Materials::Plastic.apply();    // Thân tàu xanh
        else if (i == 2 || i == 3) Materials::ToyRed.apply(); // Bu?ng lái & Mái ??
        else if (i == 4) Materials::ToyBlack.apply();      // ?ng khói xám
